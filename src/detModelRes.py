...
def processRes(obj, model):
    ...
    opr_var = ['prod', 'cur', 'state', 'spill', 'exp', 'imp', 'rat',
               'to_storage', 'from_storage', 'storage']
    ...
    obj.inv_line['Cap'] = obj.inv_line['new_branch']*obj.inv_line['MaxCap']
    obj.inv_line['Cap'] = obj.inv_line['Cap'].fillna(obj.inv_line['MaxCap'])
    obj.inv_line.index = [i[0] for i in obj.inv_line.index] 

    obj.opr_res.sort_index(axis=1,inplace=True)
    obj.opr_res.index = pd.date_range(start=obj.start_date, end=obj.end_date,
                                      freq='h')[obj.opr_res.index]
