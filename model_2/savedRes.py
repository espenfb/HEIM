...
class savedRes(object):
    ...
    def getInv(self, objects = None, cap_type = 'power', default_objects = None):
        ...
        if cap_type == 'power': inv['retired_cap'] *= -1
        return inv
    
    def getInvByType(self, objects = None, cap_type = 'power', lower_lim = 0.0,
                     default_objects = None, conH2power = False):
        ...
        inv_type = inv.rename(self.PLANT_TO_TYPE).reset_index().groupby('index')
    ...
    def getInvByNode(self, nodes = None, cap_type = 'power', lower_lim = 0.0):
        ...
        for i in nodes:
            ...
            inv.index = pd.MultiIndex.from_product([[i],inv.index.get_level_values(0).tolist()])
    ...
    def getValue(self, val_type, objects = None, times = idx[:],
        ...
        if by_node: vals.columns = [int(i[-2:]) for i in vals.columns.get_level_values(0)]
        return vals
    ...
    def getValueByType(self, val_type, objects = None, times = idx[:],
        ...
        vals = self.getValue(val_type, objects = objects,
                             times = times).rename(self.PLANT_TO_TYPE,
                                          axis = 1).reindex(axis = 1)
    ...
    def getValueByNode(self, val_type, nodes = None, objects = None,
        ...
        for i in nodes:
            ...
            vals = self.getValue(val_type, objects = obj_filt, times = times,
                      lower_lim = lower_lim)
            vals.columns = pd.MultiIndex.from_product([[i],vals.columns.get_level_values(0).tolist()])
    ...
    def getWeightedValue(self, val_type, weight_type, val_objects = None,
        ...
        if weight_coeff == None:
            weight = self.getValue(weight_type, objects = weight_objects)
            weight = weight.droplevel(1,axis = 1)
        else:
            weight = self.getProd(weight_type, weight_coeff, objects = weight_objects)
    ...
    def getLineInv(self, nodes = None, aggr = True):
        ...
        index = self.inv_line.loc[idx[:],'From'].isin(nodes)&self.inv_line.loc[idx[:],'To'].isin(nodes)
        ...
        return lines
        # if cap_type == 'Cap':
        #     return self.inv_line.loc[index,['Cap']].Cap
        # elif cap_type == 'MaxCap':
        #     return self.inv_line.loc[index,['MaxCap']].MaxCap
    ...
    def plotValueByNode(self,val_type, nodes = None, kind = 'bar',
        ...
        val_by_node.unstack().T.plot(kind = kind)
        ax = plt.gca()
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
    ...
    def plotValueByType(self, val_type, objects = None, times = idx[:],
        ...
        val_by_type.plot(kind = kind, **kwargs)
        ax = plt.gca()
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
    ...
    def getLineRes(self):
        self.addGeoLine()  
        line_res_geo = gpd.GeoDataFrame(self.inv_line.groupby(['From','To']).agg({'Cap':'sum', 'geometry': 'first'}))
        return line_res_geo    
    ...
    def plotMap(self, linetype = None, linestyle = 'color', line_color = 'k',
        ...
        if not ax and not fig:
            fig, axes = plt.subplots(nrows=1,
                                     ncols= 1, #len(linetype),
                                     sharey=True)#, constrained_layout=True)
            fig.canvas.set_window_title('Map ' + ' ' + str(nodetype))# + ' - ' + linetype)
            ax = axes
        else:
            ax.set_aspect('equal')
        ...
        if objects:
            ...
            if not max_node: max_node = size.max()
            rel_size = size/max_node
            self.data.bus.plot(ax = ax, color = node_color, hatch = hatch,
                               edgecolors = edgecolors, alpha=alpha, linewidths = 3.0,
                                markersize = rel_size*mkr_scaling, marker = marker, zorder = 5)
            if plot_legend:
                ord_mag = len(str(int(max_node)))
                area_range = (np.array([0.3, 0.6, 1.0])*max_node).round((ord_mag -2)*-1 )
                points = []
                label = []
                for area in area_range:
                    points.append(plt.scatter([], [], color = node_color, hatch = hatch,
                                              edgecolors = edgecolors, alpha = alpha, linewidths =3.0,
                                              s=(area*mkr_scaling)/max_node, marker = marker))
                    label.append(str(int(area*unit_weight)))# + ' ' + unit)
                fig.legend(points, label,scatterpoints=1, frameon=False, labelspacing=1.1,
                           title=self.PLANT_TO_TYPE[objects[0]] + end_label + ' (' + unit + ')',
        ...
        if labels:
            for k, row in self.data.bus.iterrows():
                ax.annotate(s = k, xy = (row['xy'][0]+textoffset,row['xy'][1]+textoffset), color = textcolor,
        ...
        if linetype == None:
            ...
            line_data = gpd.GeoDataFrame(line_data.groupby(['From','To']).agg(agg_by))
            ...
            if linestyle == 'color':
                ...
                print_cbar = False
                if not max_line: max_line = line_data.Cap.max()
                ...
                if plot_legend:
                    ...
                    area_range = np.nan_to_num(np.array([0.3, 0.6, 1.0])*max_line).round((ord_mag -2)*-1)
                    points = []
                    label = []
                    for area in area_range:
                        print(area)
                        plt.plot([], [], c = line_color,
                                      ...
                                      label =  t_unit_format % (area*unit_t_weight))# + ' ' + trans_unit)
    ...
    # def getEventEndAndDur(self, val, lim, objects = None, kind = 'upper'):
        
    #     val = b.getValue('storage', objects = 'BATTERY_STORAGE')
    ...
    #         vg = v.groupby((event != event.shift(1)).cumsum()).cumcount()+1
    #         vg_1 = vg.shift(-1,freq = 'h')
            
    #         event_end_dur = pd.Series({i: vg.loc[i] for i in vg.index[:-1] if vg.loc[i] > vg_1.loc[i]})
...
# out = pd.DataFrame()    
# for n, c in enumerate(a.res):
#     k = a.range[n]
#     pemel = c.getValue('prod', objects = 'PEMEL_PLANTS', by_node = True)        
#     ts = c.getValue('to_storage', objects = 'BATTERY_STORAGE', by_node = True)
#     th = c.getValue('to_storage', objects = 'HYDROGEN_STORAGE', by_node = True)
#     wind = c.getValue('prod', objects = 'ONSHORE_WIND_POWER_PLANTS', by_node = True)
#     solar = c.getValue('prod', objects = 'SOLAR_POWER_PLANTS', by_node = True)
    
#     corr = pd.DataFrame(index = pd.MultiIndex.from_product([['bat','h2','pemel'],['wind','solar']]))
#     for i in ts.columns:
#         if i in wind.columns:
#             corr.loc[('bat','wind'),i] = ts[i].corr(wind[i])
#         if i in solar.columns:
#             corr.loc[('bat','solar'),i] = ts[i].corr(solar[i])
#     for i in th.columns:
#         if i in wind.columns:
#             corr.loc[('h2','wind'),i] = th[i].corr(wind[i])
#         if i in solar.columns:
#             corr.loc[('h2','solar'),i] = th[i].corr(solar[i])
#     for i in pemel.columns:
#         if i in wind.columns:
#             corr.loc[('pemel','wind'),i] = pemel[i].corr(wind[i])
#         if i in solar.columns:
#             corr.loc[('pemel','solar'),i] = pemel[i].corr(solar[i])
#     cm = corr.mean(axis = 1)
    
#     out = pd.concat([out, cm.to_frame(name = k)], axis = 1)